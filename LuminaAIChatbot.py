...
def ask_groq(user_input, intent): 
    reply = client.chat.completions.create(
    model= "llama-3.3-70b-versatile",
    messages= [{"role": "system", "content": f"You are a friendly chatbot. The user's intent is {intent}\n"},
...
def response(intent,user_input):
    spacers()
    groq_reply = ask_groq(user_input, intent)
    print(groq_reply)
    spacers()
